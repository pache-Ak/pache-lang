...
## 数组类型 引用 函数 的cv修饰
数组、引用、函数类型都可表示为一个指向固定位置的指针，指针的底层类型固定为const ，不能被 cv 修饰。
## 在模板中用cv修饰时？
...
## mut 关键字
mut 关键字用于声明一个变量为可变的，即可以修改其值。

借鉴于 cpp 的 const，
在现代cpp中，const关键字只指示不可当前符号表的变量不可修改，
而不影响变量实际内存的修改。

constinit 关键字用于声明一个变量为常量初始化变量，
## 
